...
public class CameraFollowPlayerMovement : MonoBehaviour {
	...
	PlayerStats stats;
	public BoxCollider boundingCube;
	...
	void Start () {
		...
		levelBounds = boundingCube.bounds;
	...
	void LateUpdate () {
		...
		if (playerMovement) {
			...
			playerAliveTime = .5f;
		} else if (playerAliveTime <= 0 && Nest.currentlyActiveNest) {
		...
		targetPosition.x = Mathf.Clamp(
			targetPosition.x,
			levelBounds.center.x - .5f * levelBounds.size.x + .5f * orthoBounds.size.x,
			levelBounds.center.x + .5f * levelBounds.size.x - .5f * orthoBounds.size.x);
